...
export function canAccessAnyZone(user: ZoneAwareUser | undefined, zoneIds: Array<string | null | undefined>): boolean {
  if (zoneIds.length === 0) return true;
  return zoneIds.some((zoneId) => canAccessZone(user, zoneId));
}

export function assertAnyZoneAccess(
  user: ZoneAwareUser | undefined,
  zoneIds: Array<string | null | undefined>,
  context?: string,
): void {
  if (canAccessAnyZone(user, zoneIds)) return;
  const suffix = context ? ` (${context})` : '';
  throw new ForbiddenException(`Access denied for zones ${zoneIds.filter(Boolean).join(', ')}${suffix}`);
}
